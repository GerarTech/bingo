...
// Update bot config
export async function updateBotConfig(commands: Record<string, string>) {
  const { error } = await supabase
    .from('bot_config')
    .update({ commands, updated_at: new Date().toISOString() })
    .eq('id', 'main');
...
// Update bot messages
export async function updateBotMessages(messages: Record<string, string>) {
  const { error } = await supabase
    .from('bot_messages')
